...
export const zh = {
  ...
  'card.llmSourceHost': '宿主模型组（DSH 已配置的组）',
  'card.llmSourceHostHint': '使用 DSH 当前会话选中的模型组执行审批判定；下方 Provider / Model 留空即跟随会话当前选择，填写则覆盖。',
  'card.llmProvider': 'Provider（留空 = 会话当前模型组）',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'card.llmSourceHost': "Host model group (DSH's configured groups)",
  'card.llmSourceHostHint': 'Runs approval grading through the model group the session currently uses; leave Provider / Model empty to follow the session selection, or fill them to override.',
  'card.llmProvider': 'Provider (empty = session model group)',
...
export const ja: Record<keyof typeof zh, string> = {
  ...
  'card.llmSourceHost': 'ホストモデルグループ（DSH の設定済みグループ）',
  'card.llmSourceHostHint': 'DSH のセッションで現在選択中のモデルグループで審査判定を実行します。Provider / Model を空のままにするとセッション選択に追従し、記入すると上書きします。',
  'card.llmProvider': 'Provider（空 = セッションのモデルグループ）',
...
export const ko: Record<keyof typeof zh, string> = {
  ...
  'card.llmSourceHost': '호스트 모델 그룹(DSH에 설정된 그룹)',
  'card.llmSourceHostHint': 'DSH 세션에서 현재 선택한 모델 그룹으로 승인 판정을 수행합니다. Provider / Model을 비워 두면 세션 선택을 따르고, 입력하면 덮어씁니다.',
  'card.llmProvider': 'Provider(비움 = 세션 모델 그룹)',
