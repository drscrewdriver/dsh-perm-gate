...
export const zh = {
  ...
  // Rule test (dry-run) panel. `%d` is the defaultAction, `%s` is unused here.
  'card.dryRun': '规则测试',
...
export const ja: Record<keyof typeof zh, string> = {
  ...
  'card.rules': '権限 YAML',
  'card.rulesHint': 'ゲートが今読み込んでいるルールファイルの原文です。読み取り専用 — ここからは何も変更できません。変更するには下の許可リストを使うか、ファイルを直接編集してください。',
  'card.rulesRefresh': '再読み込み',
  'card.rulesLoading': '読み込み中…',
  'card.rulesPath': 'ファイル',
  'card.rulesNone': 'ルールファイルが設定されていません',
  'card.rulesCounts': 'ルール',
  'card.rulesDefault': 'defaultAction',
  'card.rulesStats': '%b バイト · %l 行',
  'card.rulesMissing': 'ファイルが存在しません。ゲートは組み込みの既定値にフォールバックします。',
  'card.rulesTruncated': 'ファイルが大きいため、先頭 512 KB のみ表示します。',
  'card.rulesError': '読み込みに失敗しました：',
  'card.rulesStale': 'ルートが存在しません（プラグイン未再起動？）',
  'card.rulesNote': '読み取り専用：このパネルからファイルは変更できません。書き込むのは許可リストのセクションだけです。',
}
...
export const ko: Record<keyof typeof zh, string> = {
  ...
  'card.rules': '권한 YAML',
  'card.rulesHint': '게이트가 지금 불러오는 규칙 파일 원문입니다. 읽기 전용 — 여기서는 아무것도 바꿀 수 없습니다. 바꾸려면 아래 허용 목록을 쓰거나 파일을 직접 편집하세요.',
  'card.rulesRefresh': '새로 고침',
  'card.rulesLoading': '읽는 중…',
  'card.rulesPath': '파일',
  'card.rulesNone': '규칙 파일이 설정되지 않았습니다',
  'card.rulesCounts': '규칙',
  'card.rulesDefault': 'defaultAction',
  'card.rulesStats': '%b 바이트 · %l 줄',
  'card.rulesMissing': '파일이 없습니다. 게이트는 내장 기본값으로 대체합니다.',
  'card.rulesTruncated': '파일이 커서 앞 512 KB만 표시합니다.',
  'card.rulesError': '불러오기 실패: ',
  'card.rulesStale': '라우트가 없습니다(플러그인 미재시작?)',
  'card.rulesNote': '읽기 전용: 이 패널은 파일을 수정할 수 없습니다. 쓰기는 허용 목록 섹션뿐입니다.',
}