...
export const ja: Record<keyof typeof zh, string> = {
  ...
  'card.rulesHint': 'ゲートが今読み込んでいるルール（dsh-perm-gate-rules 設定名前空間から）。読み取り専用 — ここからは何も変更できません。変更するには下の許可リストを使うか、settings.yaml を編集してください。',
...
export const ko: Record<keyof typeof zh, string> = {
  ...
  'card.rulesHint': '게이트가 지금 불러오는 규칙(dsh-perm-gate-rules 설정 네임스페이스에서). 읽기 전용 — 여기서는 아무것도 바꿀 수 없습니다. 바꾸려면 아래 허용 목록을 쓰거나 settings.yaml을 편집하세요.',
