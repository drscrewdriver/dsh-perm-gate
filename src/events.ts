...
interface RouteRes {
  ...
  /** Present on a real `http.ServerResponse`; a stream route needs it, the JSON routes do not. */
 ...
 * unavailable — the host keeps running, only the HTTP API is missing.
 */
export function registerStreamRoute(server: unknown, log: EventLog): (() => void) | undefined {
  const ws = routeServer(server)
  if (ws === undefined) return undefined
  return ws.register({
    kind: 'exact',
    ...
    handler: (rawReq, rawRes) => {
      ...
      const push = (events: readonly GateEvent[], backlog = false): void => {
        ...
        if (events.length === 0 && !backlog) return
        try {
      ...
      const unsubscribe = log.subscribe((event) => {
        if (scoped && event.sessionId !== sessionId) return
        push([event])
      })
