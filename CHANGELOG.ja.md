...
- **入力欄グリフのパッチが DSH 0.1.5 で再び機能します。** `scripts/patch-permission-glyph.mjs`
  は `["permissive",` の直後にエントリを挿入していましたが、そのキーは DSH 0.1.2 がグリフを
  描いていた値で、0.1.5-rc.2 のバンドルにはどこにも存在しません。そのためパッチは
  `anchor not found` で失敗し、両ティアはテキストのみのままでした。取得元はティアごとに
  `GLYPH_TARGETS` で宣言し、アンカーは**解決**します —— まず `["<key>",`、次にバンドルが使う
  定数別名（`danger-full-access` は `[FULL_ACCESS,`）。ホストが描かないキーは、Map が実際に
  持つキーと一緒に報告されます。
- **UI を提供していない DSH インストールにパッチを当てなくなりました。** 候補は profile
  スコープを先、CLI インストールを次に並べ、同じバンドルを持つ**他の**インストールは黙って
  見送らず出力に列挙します。このプロジェクトのホストで実測：グローバル CLI インストールが
  ランチャー管理のランタイムを隠していたため、パッチは成功と報告しつつピッカーは
  アイコンなしのままでした。

### 変更

- **各ティアは、共有するファイルサンドボックスのグリフを付けます（共通の盾＋目ではなく）。**
  「自动审查」は `workspace-write` のグリフを、「自动审查（高权限）」は `danger-full-access` の
  グリフをコピーするため、アイコンはそのティアが実際に与えるアクセスと一致します。同一アイコンの
  旧挙動は、`permissive` のグリフを持っていた 0.1.2 の Map に由来します。
- **2 つのグリフは両方適用するか、両方とも適用しません。** 解決できない取得元キーは何も書かない
  拒否であり、部分的にパッチされたバンドルにはなりません。
...
- **`main` ラインのバージョン系列を `2.x` に変更し、対応する DSH ラインを表すようにしました。**
  `1.x` は DSH `<= 0.1.1` ライン、`2.x` は DSH `0.1.2+` ラインであり、プラグインのバージョンが
  どの DSH 向けかを示します。`2.0.0` が新系列の最初のリリースで、`0.2.x`
  （`0.2.0`、`0.2.1-beta.2`…`beta.5`）はこれに置き換わります。メジャー同士は互いを遮断するため
  （`^1.0.0` は `2.0.0` を解決せず、`^2.0.0` は `1.0.0` を解決しません）、既存の
  `^0.2.1-beta.4` インストール（これも `2.0.0` を解決しません）は明示的に更新する必要があります。
  `engines.dsh` も同じ分界を述べています（こちらは `>=0.1.2-alpha.1 <0.2.0-0`、`legacy` は
  `>=0.1.0-rc.7 <0.1.2-alpha.1`）が、DSH は決して読まないため、古い DSH を `1.x` に留めるのは
  バージョン範囲と dist-tag です。
