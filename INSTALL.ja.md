...
ルール本体は `dsh-perm-gate-rules` settings 名前空間に保存され、変更のたびにゲートが
その場で再コンパイルされます。上記の YAML ファイルは任意です。名前空間が未設定の間
だけ読み込まれ、プラグインが最初にルールを書き込む際（許可リストの承認、または設定
UI での編集）にファイルのルールが自動的に取り込まれます。
