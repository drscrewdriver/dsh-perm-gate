 ...
 */
export function classifySessions(raw) {
    let doc;
    try {
        doc = JSON.parse(raw);
    }
    ...
    if (typeof workspaces === 'object' && workspaces !== null) {
        for (const record of Object.values(workspaces)) {
            ...
            if (!Array.isArray(ids))
                continue;
            for (const id of ids)
                live.add(normalizeSessionId(id));
        }
    }
    const archived = new Set();
    if (Array.isArray(root.global?.archivedSessionIds)) {
        for (const id of root.global.archivedSessionIds)
            archived.add(normalizeSessionId(id));
    }
 ...
 */
export function isStaleSession(id, cls) {
...
function sweepSnapshots({ classification, snapshotsDir }) {
    let entries;
    try {
