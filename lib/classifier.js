...
async function postChat(url, headers, body, signal, nowFetch) {
    try {
 ...
 */
export async function chatCompletion(cfg, system, user, nowFetch = fetch) {
 ...
 */
export async function withLlmRetry(task, attempts = 2) {
 ...
 */
export async function classifyWithLLM(cfg, req, nowFetch = fetch) {
    const system = 'You are a permission classifier for a coding agent. Judge one tool call and reply with ONLY JSON ' +
