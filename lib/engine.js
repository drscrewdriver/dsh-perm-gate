...
const NO_EXTRA_AUTO_ALLOW = new Set();
/** The built-in read-only / internal classification, for diagnostics and docs. */
export const AUTO_ALLOW_TOOLS = new Set([...READ_TOOLS, ...INTERNAL_TOOLS]);
/**
 ...
 */
export const CONTENT_ARG_KEYS = new Set([
...
function operationText(args, depth = 0) {
    if (depth > 6)
        return '';
    const parts = [];
        ...
        if (Array.isArray(value)) {
            for (const item of value) {
                if (typeof item === 'string')
                    ...
                    parts.push(operationText(item, depth + 1));
            }
            continue;
        }
        if (value !== null && typeof value === 'object') {
            parts.push(operationText(value, depth + 1));
        }
    }
    return parts.join(' ');
}
function containsCredentialMaterial(args) {
 ...
 */
function gitRemoteRewriteReason(segments) {
    for (const seg of segments) {
        const text = [seg.command, ...seg.args].join(' ');
        if (!GIT_PUSH_TEXT.test(text))
            continue;
        const result = dispatchCommand(text);
        if (result === null)
            continue;
        const s = result.semantics;
        if (s.operation !== 'force-push' && s.operation !== 'push-delete')
            continue;
        if (!s.targetsSharedBranch)
            continue;
 ...
 * later stages.
 */
export function hardDenyReason(ctx) {
    const args = ctx.args;
    if (containsCredentialMaterial(args))
        return 'call contains credential or private-key material';
    if (SHELL_TOOLS.has(ctx.tool) && typeof args.command === 'string') {
        ...
        for (const cmd of commands) {
            ...
            if (redir !== undefined)
                return `shell redirect targets protected path: ${redir}`;
        }
    ...
    if (path !== undefined && (DESTRUCTIVE_TOOL.test(ctx.tool) || ctx.tool === 'bash' || ctx.tool === 'pwsh' || ctx.tool === 'write' || ctx.tool === 'edit')) {
        if (isProtectedDestructiveTarget(path, ctx.dshHome))
            return `mutation targets protected path: ${path}`;
    }
...
export function decide(ctx, ruleset, grants, 
/** Extra tool names the user classified as safe (`autoAllowTools`). */
autoAllowExtra = NO_EXTRA_AUTO_ALLOW) {
    const hard = hardDenyReason(ctx);
    if (hard !== undefined)
        return { action: 'deny', reason: `[hard-deny] ${hard}`, stage: 'hard-deny', ruleIndex: undefined };
