...
import { appendAllowToSettings, replaceAllowInSettings } from './allowlist.js';
import { runDryRun } from './dry-run.js';
import { registerDryRunRoute, registerEventsRoute, registerHealthRoute, registerLearningRoute, registerNetworkRoute, registerReceiverRoute, registerReviewRoutes, registerRulesRoute, registerStreamRoute } from './events.js';
...
export function apply(ctx, config = {}) {
    ...
    let rulesFromSettings = () => ({});
    const rulesScopeRef = { scope: undefined };
    ...
    const settingsWrite = (write, fallback) => {
        const scope = rulesScopeRef.scope;
        if (scope === undefined)
            return fallback();
        void write(scope, fileSeedDoc()).catch((e) => {
    ...
    const runtime = new PermGateRuntime({
        ...
        allowlistWriter: {
            append: (pattern, reason) => settingsWrite((scope, seed) => appendAllowToSettings(scope, pattern, reason, seed), () => false),
            replace: (patterns, reason) => settingsWrite((scope, seed) => replaceAllowInSettings(scope, patterns, reason, seed), () => false),
        },
    ...
    installSettingsSection(ctx, RULES_NAMESPACE, RulesSchema, {}, {
        setSource: (source) => {
    ...
    try {
        ...
        if (webServer !== undefined && runtime.eventLog !== undefined) {
            ...
            if (offStream !== undefined)
                ctx.effect(() => () => { offStream(); }, 'dsh-perm-gate: stream route');
        }
        ...
        if (webServer !== undefined) {
            ...
            const offRules = registerRulesRoute(webServer, {
                view: () => {
                    ...
                    if (isRulesConfigured(doc))
                        return readRulesViewFromSettings(doc);
