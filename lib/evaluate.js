...
import { extractPathCandidates, extractUrlCandidates, } from './rule.js';
import { commandArgv, decomposeShellCommand, isForceDeletion, isRecursiveDeletion } from './shell.js';
...
export function ruleMatches(rule, ctx, commands, pathCands, urlCands) {
    if (!rule.enabled)
        return false;
    // tools dimension: any glob matches.
    if (rule.tools.length > 0 && !rule.tools.some((t) => t.re.test(ctx.tool)))
        return false;
    ...
    if (rule.command.length > 0) {
        ...
        for (const spec of rule.command) {
            for (const cmd of commands) {
                if (!spec.word.re.test(cmd.command))
                    continue;
                if (spec.flag === 'recursive' && !isRecursiveDeletion(cmd))
                    continue;
                if (spec.flag === 'force' && !isForceDeletion(cmd))
                    continue;
                ok = true;
                break;
            }
    ...
    if (rule.args.length > 0) {
        const tokens = [];
        for (const cmd of commands)
            tokens.push(...cmd.args, ...cmd.redirects);
        tokens.push(...pathCands, ...urlCands);
        if (!tokens.some((tok) => rule.args.some((g) => g.re.test(tok))))
            return false;
    }
    ...
    if (rule.agents.length > 0) {
        ...
        if (!candidates.some((c) => rule.agents.some((a) => a === c || matchAgentPattern(a, c))))
            return false;
    }
    // when dimension: all conditions must be satisfied.
    if (rule.when !== undefined) {
        if (!matchWhen(rule.when))
            return false;
    }
    ...
    if (rule.branch !== undefined) {
        if (!matchBranch(rule.branch, commands))
            return false;
    }
    return true;
}
...
function isWriteCommand(commandText) {
    return SHELL_WRITE_PATTERNS.some((p) => typeof p === 'string' ? commandText.includes(p) : p.test(commandText));
}
/**
 ...
 */
function pathAndCommandDefault(ctx, pathCandidates, defaultAction) {
    ...
    if (WRITE_TOOLS.has(ctx.tool)) {
        ...
        if (home !== undefined && home !== '') {
            for (const candidate of pathCandidates) {
                if (normalizeWorkspacePath(home, candidate, ctx.caseInsensitive) === '')
                    return 'ask';
            }
        }
        return defaultAction;
    ...
    if (SHELL_TOOLS.has(ctx.tool)) {
        ...
        if (commandText !== undefined && isWriteCommand(commandText))
            return 'ask';
    }
    return defaultAction;
}
 ...
 */
function commandTextOf(ctx) {
    if (typeof ctx.commandText === 'string')
        return ctx.commandText;
    const raw = ctx.args?.['command'];
    return typeof raw === 'string' ? raw : undefined;
}
/** Compute the deterministic first-match decision (P2 chain). */
export function decideRules(ruleset, ctx) {
...
function compileGlobForMatch(pattern) {
 ...
 */
function matchParams(params, args) {
    for (const cond of params) {
        ...
        if (cond.negated) {
            ...
            if (compiled[0].re.test(strValue))
                return false;
        }
        else {
            // Normal: value must match at least one pattern (OR).
            if (!compiled.some((c) => !c.negated && c.re.test(strValue)))
                return false;
        }
    }
    return true;
 ...
 */
function matchAgentPattern(pattern, candidate) {
...
function matchWhen(when) {
    ...
    if (when.platform !== undefined && when.platform.length > 0) {
        if (!when.platform.includes(process.platform))
            return false;
    }
    // Env var checks.
 ...
 */
function matchNetwork(network, ctx) {
    if (ctx === undefined)
        return false;
    // domains
    if (network.domains !== undefined && network.domains.length > 0) {
        ...
        if (!matchers.some((m) => m.re.test(ctx.domain)))
            return false;
    }
    // ips
    ...
    if (network.schemes !== undefined && network.schemes.length > 0) {
        if (ctx.scheme === undefined)
            return false;
        if (!network.schemes.includes(ctx.scheme))
            return false;
    }
    return true;
}
 ...
 */
const commandSemanticsCache = new WeakMap();
function semanticsOf(cmd) {
 ...
 */
function matchBranch(branch, commands) {
    if (commands.length === 0)
        return false;
    const branches = [];
    const remotes = [];
    ...
    for (const cmd of commands) {
        ...
        if (result === null || result.semantics.family !== 'git')
            continue;
        const { branch: name, remote, targetsSharedBranch } = result.semantics;
        if (targetsSharedBranch)
            anyShared = true;
    ...
    if (branch.shared && !anyShared)
        return false;
    if (branch.target.length > 0 && !branches.some((b) => branch.target.some((g) => g.re.test(b))))
        return false;
    if (branch.remote.length > 0 && !remotes.some((r) => branch.remote.some((g) => g.re.test(r))))
        return false;
    return true;
}
