 ...
 */
export function detectShadows(ruleset) {
    const shadowed = [];
 ...
 */
function isDominated(dominator, candidate) {
    ...
    if (dominatorBroad !== undefined)
        return dominatorBroad;
    return undefined;
}
/**
 * Check if dominator is strictly broader than candidate on at least one dimension.
 */
function isStrictlyBroader(dominator, candidate) {
