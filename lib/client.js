window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"card.rulesHint": "门禁此刻实际加载的规则（来自 settings 命名空间 dsh-perm-gate-rules）。只读——这里改不了任何东西。要改规则，用下面的白名单，或在 settings.yaml 中编辑。",
		...
		const en = {
			...
			"card.rulesHint": "The rules the gate is loading right now (from the dsh-perm-gate-rules settings namespace). Read-only — nothing here can change them. To change rules, use the allowlist below or edit settings.yaml.",
