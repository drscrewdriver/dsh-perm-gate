window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"card.rules": "权限 YAML",
			"card.rulesHint": "门禁此刻实际加载的规则文件原文。只读——这里改不了任何东西。要改规则，用下面的白名单，或直接编辑该文件。",
			"card.rulesRefresh": "刷新",
			"card.rulesLoading": "读取中…",
			"card.rulesPath": "文件",
			"card.rulesNone": "未配置规则文件",
			"card.rulesCounts": "规则",
			"card.rulesDefault": "defaultAction",
			"card.rulesStats": "%b 字节 · %l 行",
			"card.rulesMissing": "文件不存在。门禁会退到内置默认。",
			"card.rulesTruncated": "文件过大，仅显示前 512 KB。",
			"card.rulesError": "加载失败：",
			"card.rulesStale": "路由不存在（插件未重启？）",
			"card.rulesNote": "此处只读：面板无法修改该文件，白名单段是唯一会写入它的地方。"
		};
		...
		const en = {
			...
			"card.rules": "Permissions YAML",
			"card.rulesHint": "The rules file the gate is loading right now, verbatim. Read-only — nothing here can change it. To change rules, use the allowlist below or edit the file directly.",
			"card.rulesRefresh": "Refresh",
			"card.rulesLoading": "Reading…",
			"card.rulesPath": "File",
			"card.rulesNone": "no rules file configured",
			"card.rulesCounts": "Rules",
			"card.rulesDefault": "defaultAction",
			"card.rulesStats": "%b bytes · %l lines",
			"card.rulesMissing": "The file does not exist. The gate falls back to its built-in defaults.",
			"card.rulesTruncated": "File is large; showing the first 512 KB only.",
			"card.rulesError": "Load failed: ",
			"card.rulesStale": "route missing (plugin not restarted?)",
			"card.rulesNote": "Read-only: this panel cannot modify the file. The allowlist section is the only place that writes to it."
		};
		//#endregion
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
					...
					children: unavailable ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						value.networkEnabled === true ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
							style: sectionStyle,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
									style: labelStyle,
