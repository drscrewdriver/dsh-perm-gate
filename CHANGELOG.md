...
- **Session sweep — the authorization chain now follows the session lifecycle.** On plugin startup
  ...
  every I/O failure is fail-open; the hourly timer is `unref`'d and disposed with the plugin.
  New config: `sessionSweep` (default `true`), `workspaceStoreFile`. Restoring an archived session
  does not restore its swept history.
