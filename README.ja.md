...
> | 0.1.2-alpha.1 ~ 0.1.5-rc.0 | `main` | `2.x` | `@latest` / `@dsh-0.1.2` （`@2.x` は範囲） |
> | 0.1.5-rc.1+ | `sync/0.1.5-from-main`（=`compat/0.1.5`） | `3.x` | GitHub ref インストール（dist-tag 未公開） |
>
...
バージョン **2.6.1** — 変更履歴は [日本語 changelog](./CHANGELOG.ja.md) を参照。
...
だけで、P0 ハード拒否は**ゲート自身のスコープ内で**単調かつ交渉不能のままです。

> **P0 はスコープ付きであり、グローバルではありません。** このゲートはセッションの権限プリセットが
> `gatePresets`(既定は `permissive` / `permissive-full`)のいずれかである間だけ動作します。それ以外の
> プリセット —— 読み取り専用 / ワークスペース内変更 / 完全権限 —— では **P0 ハード拒否を含むゲート
> 全体が停止**し、選択されたティア自身のポリシーがそのセッションを統治します。これは意図的な設計です
> (設定表の `gatePresets` を参照)が、「P0 は交渉不能」は**ゲートのティア内**で成立するものであり、
> すべてのティアに跨るものではありません。停止は**沈黙しません**: セッション/プリセットの遷移ごとに
> `stand-down` イベントを 1 件記録し、ブラウザは入力欄の上に **GATE OFF** の常駐ストリップを表示します。
> `gatePresets: ['*']` に設定すると P0 は再びグローバルになります。
...
*host-configured names outside the design set get none*。どちらのティアも組み込み値ではないため、
アイコンは `npx dsh-perm-gate-patch-glyph` がその表に 2 項目を足すことで初めて付きます ——
「自动审查」は `workspace-write` の盾＋鉛筆を、「自动审查（高权限）」は `danger-full-access` の
盾＋感嘆符をコピーし、それぞれが実際に共有するファイルサンドボックスのグリフになります。このパッチは
...
拒否も、P0 ハード拒否も、拒否キーワードの遮断も行いません。選択された
ティア自身の方針が呼び出しを決めます。ただし**沈黙はしません**: セッション/プリセットの遷移ごとに
`stand-down` イベントを 1 件だけ記録し（判定ではなく通知です）、ブラウザは入力欄の上に
**GATE OFF** の常駐ストリップを表示します。`danger-full-access` の定義は「承認プロンプトなしの
...
- `risky` + **ハードリスクカテゴリ**（`deletion`、`credential`、`remote`、`system`、`bulk`）→ **人間への確認（ask）を維持**します。分類器は**決して拒否しません**。拒否は決定論的レイヤー（P0 ハード拒否、拒否キーワードのブラックリスト、明示的な `deny:` ルール）だけに属するため、誤判定されたカテゴリは常に交渉可能で、異議を唱えられないブロックにはなりません。ハードカテゴリが `neutral` と異なる重要な点は一つだけです: **決して学習されない**ため、承認を繰り返しても自動許可に沈殿しません。
  （モデルの判断を拒否の根拠にした結果は実測済みです: 無害な `git commit -F …` が `remote` と判定され、パネルも再試行用の grant もない自動拒否が発生しました。）
...
「自动审查」と「自动审查（高权限）」はどちらもピッカーにアイコンを描きます —— インストール
ガイドに書かれたホストパッチが、それぞれが共有するファイルサンドボックスのティア
（`workspace-write` と `danger-full-access`）からコピーしたものです。そのパッチが
ない場合、両ティアはどのサーフェスでもテキストのみになります。ラベルとゲートは影響を
受けません。
